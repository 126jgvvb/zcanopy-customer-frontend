    ...
    <section className="rounded-2xl border border-[var(--zcanopy-border)] bg-[var(--zcanopy-surface)] p-6 shadow-[var(--zcanopy-shadow-sm)]">
      {(title || action) && (
        <div className="mb-5 flex items-center justify-between gap-3">
          {title ? (
            <h2 className="text-base font-semibold tracking-tight text-[var(--zcanopy-card-brown)]">{title}</h2>
          ) : (<span />)}
          {action}
        </div>
      )}
...
export function StatCard({ label, value, hint }: { label: string; value: ReactNode; hint?: string }) {
  return (
    <div className="relative overflow-hidden rounded-2xl border border-[var(--zcanopy-border)] bg-[var(--zcanopy-surface)] p-5 shadow-[var(--zcanopy-shadow-sm)] transition-shadow duration-200 hover:shadow-[var(--zcanopy-shadow-md)]">
      ...
      <p className="mt-3 text-2xl font-semibold tracking-tight text-[var(--zcanopy-card-brown)]">{value}</p>
...
export function ErrorState({ message }: { message: string }) {
  return (
    <div className="rounded-xl border border-red-200/80 bg-red-50/90 px-4 py-3 text-sm text-red-700">{message}</div>
  );
}